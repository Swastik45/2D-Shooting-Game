...
pub const TILE_SIZE: f32 = 48.0; // render size (upscaled from 16px source)
const TILE_SOURCE: u32 = 16;     // source tile px

// Tile IDs matching tileset.png columns
const G: u8 = 0;  // grass
const D: u8 = 1;  // dirt
...
const MAP: [[u8; MAP_W]; MAP_H] = [
    ...
    [TR, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G,TR,TR, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G, G,TR],
    [TR, G,RF,RF,RF,RF, G, G,RF,RF,RF,RF, G, G, G, G, G, G, G,TR,TR, G, G, G, G, G,RF,RF,RF,RF, G, G,RF,RF,RF,RF, G, G, G,TR],
...
pub fn spawn_world(
    ...
    // Map origin: center the map around world (0,0)
    ...
    for row in 0..MAP_H {
        for col in 0..MAP_W {
            ...
            commands.spawn((
                ...
                Transform::from_xyz(x, y, -1.0), // z=-1 so player renders on top
                TileMap,
            ));
        }
    }
